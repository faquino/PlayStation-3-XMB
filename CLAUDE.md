...
- **Verified:** `runTask` is the SPU update task of `particles.elf`, steps 1–8 of `PARTICLES_REVERSE_ENGINEER.md` in order: flow grid sample, noise from three generators reseeded every frame, field rotation, vec4 integration (life in `position.w`, aging rate in `velocity.w`), quaternion spin, death back to the free list, vertex record. Its constants (`FREE = -666`, `LIFE_END`, `NOISE_SEEDS`, `SPIN_FREQS`, the 13 × 8 grid, `LIFE_MIN`/`LIFE_MAX`, `CAMERA`) are traced, not free parameters — reconcile with the doc before changing them.
- **Modelled:** the PPU side, not traced yet: `buildParams` (which `.mnu` value goes where, and the input response), `emitOne` and `buildFlowGrid`. Its constants (`WAVE_DEPTH_NEAR`/`FAR`, `EMIT_EXTENT`, `FIELD_CENTRE`) are calibrated against the RSX captures, and the doc records how.
