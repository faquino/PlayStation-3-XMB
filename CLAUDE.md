...
- **Verified:** `runTask` is the SPU update task of `particles.elf`, steps 1–8 of `PARTICLES_REVERSE_ENGINEER.md` in order: flow grid sample, noise from three generators reseeded every frame, field rotation, vec4 integration (life in `position.w`, aging rate in `velocity.w`), quaternion spin, death back to the free list, vertex record. Its constants (`FREE = -666`, `LIFE_END`, `NOISE_SEEDS`, `SPIN_FREQS`, `LIFE_MIN`/`LIFE_MAX`, `CAMERA`) are traced, and the parameter block `createParams` describes — its 2304-byte layout, the 32 × 16 grid, `GRID_SCALE`/`GRID_ORIGIN`, `FIELD_CENTRE` — was read out of RPCS3 savestates and frame captures (`tools/re/readblock.py`). The PPU side is ported as far as it is traced in `custom_render_plugin`: `buildParams` fills the block its way (the wind as the noise offset, the noise's formula and spring), `decayGrid` and `iconWind` write the flow grid its way, and the icons' layout (`ICON_*`, `ITEM_*`) is measured in the captures. None of them are free parameters; reconcile with the doc before changing them.
- **Modelled:** the emitter (`emitOne`), how the icons move between their places (`moveIcons`), and what the input feeds into `buildParams`: the field's turn, the spring's impulses and the motion term. Its constants (`WAVE_DEPTH_NEAR`/`FAR`, `EMIT_EXTENT`) are calibrated against the RSX captures, and the doc records how.
